...
def plot_latlon(frame, wrfin, out, vmin=100, vmax=300):
    ...
    for n, g in frame_filtered:
        data = np.zeros((int((max_lat - min_lat) * 10) + 1, int((max_lon - min_lon) * 10) + 1))
        cnt = np.zeros((int((max_lat - min_lat) * 10) + 1, int((max_lon - min_lon) * 10) + 1))
        for i, row in g.iterrows():
            ...
            if min_lat < lat < max_lat:
                x = int((lat - min_lat) * 10)

            if min_lon < lon < max_lon:
                y = int((lon - min_lon) * 10)

            if x == -1 or y == -1:
                continue

            data[x, y] += row.surf_co[0]
            cnt[x, y] += 1
...
if __name__ == '__main__':
    ...
    plot_latlon(frame, wrfin, 'mopitt_surf_2017.png')

    with open(mop_pkl_2018, 'rb') as fp:
        frame = pickle.load(fp)

    plot_latlon(frame, wrfin, 'mopitt_surf_2018.png')

    with open(mop_pkl_2019, 'rb') as fp:
        frame = pickle.load(fp)

    plot_latlon(frame, wrfin, 'mopitt_surf_2019.png')

    with open(mop_pkl_2020, 'rb') as fp:
        frame = pickle.load(fp)

    plot_latlon(frame, wrfin, 'mopitt_surf_2020.png')
