...
def add_plot_raw(exp_type, data_2017, data_2018, data_2019, data_2020, infile, title, outfile, vmin, vmax, vstep, cmap):
    ...
    ax_2017.text(0.1, -0.12,'(a) Exp2017_%s' % exp_type, fontsize=16, weight='bold', transform=ax_2017.transAxes)
    ax_2018.text(0.1,-0.12, '(b) Exp2018_%s' % exp_type,fontsize=16, weight='bold', transform=ax_2018.transAxes)
    ax_2019.text(0.1,-0.12, '(c) Exp2019_%s' % exp_type, fontsize=16, weight='bold', transform=ax_2019.transAxes)
    ax_2020.text(0.1,-0.12, '(d) Exp2020_%s'  % exp_type, fontsize=16, weight='bold', transform=ax_2020.transAxes)
...
if __name__ == '__main__':
    ...
    with open('surf_mean_6h_2017_new_sim_1', 'rb') as fp:
        da_mean_2017 = pickle.load(fp).mean(axis=0)
    with open('surf_mean_6h_2018_new_sim_1', 'rb') as fp:
        da_mean_2018 = pickle.load(fp).mean(axis=0)
    with open('surf_mean_6h_2019_new_sim_1', 'rb') as fp:
        da_mean_2019 = pickle.load(fp).mean(axis=0)
    with open('surf_mean_6h_2020_new_sim_1', 'rb') as fp:
    ...
    # with open('prior_mean_6h_2017_new_sim', 'rb') as fp:
    #     sim_mean_2017 = pickle.load(fp).mean(axis=0)
    # with open('prior_mean_6h_2018_new_sim', 'rb') as fp:
    #     sim_mean_2018 = pickle.load(fp).mean(axis=0)
    # with open('prior_mean_6h_2019_new_sim', 'rb') as fp:
    #     sim_mean_2019 = pickle.load(fp).mean(axis=0)
    # with open('prior_mean_6h_2020_new_sim', 'rb') as fp:
    #     sim_mean_2020 = pickle.load(fp).mean(axis=0)

    # 打开文件
    inpath_2020 = r"D:\exp_data\lunwen_winter\data\Concentraion\2010\da_with_init_update\2020"
    title_2020 = 'DA XCO mean in winter 2020'
    inpath_2019 = r"D:\exp_data\lunwen_winter\data\Concentraion\2010\da_with_init_update\2019"
    title_2019 = 'DA XCO mean in winter 2019'
    inpath_2018 = r"D:\exp_data\lunwen_winter\data\Concentraion\2010\da_with_init_update\2018"
    title_2018 = 'DA XCO mean in winter 2018'
    inpath_2017 = r"D:\exp_data\lunwen_winter\data\Concentraion\2010\da_with_init_update\2017"
    title_2017 = 'DA XCO mean in winter 2017'

    # inpath_2020_new_sim = r'D:\exp_data\lunwen_winter\data\sim\2020_new_sim\%s' % type
    ...
    print(2017, 'da:', np.mean(da_mean_2017))
    print(2018, 'da:', np.mean(da_mean_2018))
    print(2019, 'da:',np.mean(da_mean_2019))
    print(2020, 'da:',np.mean(da_mean_2020))
    ...
    add_plot_raw('DA_SURF',da_mean_2017, da_mean_2018, da_mean_2019, da_mean_2020 ,files_2018[0], '',
